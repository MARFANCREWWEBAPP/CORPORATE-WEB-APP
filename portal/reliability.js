...
async function putObject(env,key,bytes,fetcher=fetch){
  ...
  const names=Object.keys(headers).sort();const canonical=['PUT',url.pathname,'',names.map(n=>n+':'+headers[n]+'\n').join(''),names.join(';'),digest].join('\n');
  ...
  const response=await fetcher(url,{method:'PUT',headers,body:bytes,signal:AbortSignal.timeout(60000)});if(!response.ok)throw new Error('El almacenamiento externo rechazó la copia ('+response.status+').');return {key,sha256:digest,sizeBytes:bytes.length};
}
function createRecovery(store,env){
  ...
  function report(){const current=status();const problems=[current.lastError?'No se pudo crear una copia local':null,current.externalError?'La copia externa necesita revisión':null,current.capacityWarning?'El almacenamiento está cerca de su límite':null].filter(Boolean);if(!problems.length)return;const today=new Date().toISOString().slice(0,10);const signature=sha(today+problems.join('|'));if(store.read().notifications.some(n=>n.serviceAlert===signature))return;store.transaction(null,'CONTINUITY_ALERT',state=>{for(const user of state.users.filter(u=>u.active&&u.role==='ADMIN')){const notification={id:crypto.randomUUID(),recipientId:user.id,eventId:null,type:'SYSTEM_ALERT',title:'Revisar conservación de datos',body:problems.join('. '),createdAt:new Date().toISOString(),readAt:null,serviceAlert:signature};state.notifications.unshift(notification);state.outbox.push({id:notification.id,recipientId:user.id,type:'notification',subject:'Marquee Flow · Revisar conservación de datos',body:notification.body,createdAt:notification.createdAt,attempts:0,nextAttemptAt:notification.createdAt});}});}
...
module.exports={context,sha,install,clientAddress,sealed,unseal,putObject,createRecovery,verifyRecovery};
