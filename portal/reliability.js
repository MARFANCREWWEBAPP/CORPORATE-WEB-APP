...
function install(Store){
  ...
  Store.prototype.transaction=function(actor,action,fn){
    ...
    return original.call(this,actor,action,state=>{
      // Recheck the route's event even when replaying a cached child record (order, change or incident).
      const eventRoute=operation.route.match(/^[A-Z]+ \/api\/events\/([^/?]+)/);
      if(eventRoute){const event=state.events.find(e=>e.id===eventRoute[1]);if(!event||!require('./store').canVenue(actor,event.venueId))throw Object.assign(new Error('Evento no encontrado.'),{status:404});}
...
function createRecovery(store,env,fetcher=fetch){
  let lastError=null,externalError=null,inFlight=false;
  const hidrive=require('./hidrive-backup').createHiDriveBackup(store,env,{sha,sealed,unseal},fetcher);
  ...
  function prune(){
    ...
    const needsHiDrive=hidrive.status().requested;
    for(const b of backups){if(keep.has(b.filename)||!['arranque','automática diaria','evento archivado','automática horaria'].includes(b.reason))continue;if(configured&&!b.external)continue;if(needsHiDrive&&!hidrive.verified(b))continue;fs.unlinkSync(path.join(store.directory,'backups',b.filename));fs.unlinkSync(path.join(store.directory,'backups',b.filename+'.json'));}
  }
  async function externalS3(){if(inFlight||!configured)return;inFlight=true;try{
    for(const b of store.backups().filter(b=>!b.external).slice(0,5)){
      ...
      const object=await putObject(env,'marquee/'+b.filename+'.enc',sealed(bytes,key),fetcher);const downloaded=await putObject(env,object.key,Buffer.alloc(0),fetcher,'GET');if(sha(downloaded)!==object.sha256||sha(unseal(downloaded,key))!==b.sha256)throw new Error('La lectura de la copia externa no coincide.');await putObject(env,'marquee/'+b.filename+'.manifest.json',Buffer.from(JSON.stringify({...b,encryptedSha256:object.sha256,encryptedObject:object.key,format:'marquee-encrypted-backup-v1'})),fetcher);const latest=JSON.parse(fs.readFileSync(filename+'.json','utf8'));latest.external={...object,uploadedAt:new Date().toISOString(),verifiedAt:new Date().toISOString()};fs.writeFileSync(filename+'.json',JSON.stringify(latest),{mode:0o600});
    }externalError=null;
  }catch(error){externalError={at:new Date().toISOString(),message:error.message};}finally{inFlight=false;}}
  async function external(){await Promise.all([externalS3(),hidrive.sync()]);try{prune();}catch{lastError=new Date().toISOString();}}
  function status(){const stats=fs.statfsSync(store.directory),free=Number(stats.bavail)*Number(stats.bsize),total=Number(stats.blocks)*Number(stats.bsize);const backups=store.backups();return {lastError,externalConfigured:configured,externalError,externalLastSuccess:backups.find(b=>b.external)?.external.uploadedAt||null,externalLastVerified:backups.find(b=>b.external?.verifiedAt)?.external.verifiedAt||null,hidrive:hidrive.status(),freeBytes:free,totalBytes:total,capacityWarning:free<Math.max(512*1024*1024,total*.1),retention:'31 puntos diarios y 12 mensuales; mínimo 5 copias recientes. Las manuales se conservan.',lastBackup:backups[0]||null};}
  function report(){const current=status();const problems=[current.lastError?'No se pudo crear una copia local':null,current.externalError?'La copia externa necesita revisión':null,current.hidrive.error?'La copia adicional en HiDrive necesita revisión':null,current.capacityWarning?'El almacenamiento está cerca de su límite':null].filter(Boolean);if(!problems.length)return;const today=new Date().toISOString().slice(0,10);const signature=sha(today+problems.join('|'));if(store.read().notifications.some(n=>n.serviceAlert===signature))return;store.transaction(null,'CONTINUITY_ALERT',state=>{for(const user of state.users.filter(u=>u.active&&u.role==='ADMIN')){const notification={id:crypto.randomUUID(),recipientId:user.id,eventId:null,type:'SYSTEM_ALERT',title:'Revisar conservación de datos',body:problems.join('. '),createdAt:new Date().toISOString(),readAt:null,serviceAlert:signature};state.notifications.unshift(notification);state.outbox.push({id:notification.id,recipientId:user.id,type:'notification',subject:'Marquee Audiovisuales · Revisar conservación de datos',body:notification.body,createdAt:notification.createdAt,attempts:0,nextAttemptAt:notification.createdAt});}});}
  return {make,prune,external,status,report,hidrive};
}
