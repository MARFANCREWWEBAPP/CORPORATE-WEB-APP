...
class PostgresDatabase{
  constructor(options){
    ...
    try{this.exec(`CREATE TABLE IF NOT EXISTS state (id INTEGER PRIMARY KEY CHECK(id=1), value JSONB NOT NULL);
      ...
      CREATE TABLE IF NOT EXISTS operations (key TEXT PRIMARY KEY, digest TEXT NOT NULL, result TEXT NOT NULL, created BIGINT NOT NULL);`);}catch(error){this.worker.terminate();this.port.close();throw error;}
  }
  rpc(request){
  ...
  sql(input){
    let sql=input.replace(/PRAGMA[^;]+;/g,'').replace(/\bBLOB\b/g,'BYTEA').replace(/expires INTEGER/g,'expires BIGINT').replace(/created INTEGER/g,'created BIGINT').replace(/value TEXT NOT NULL/g,'value JSONB NOT NULL');
